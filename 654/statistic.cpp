...
void numofbook()
{
	...
	else
	{
		...
		printf("Trong kho hien co: %d cuon sach", N);
	}
	if (f) 
	{
		fclose(f);
...
void numofbook_gerne()
{
	...
	else
	{
		...
		{
			printf("The loai %s co: %d cuon sach ", B[i].Gen, B[i].Num);
		}
	}
	if (f)
	{
...
void numofsubscriber()
{
	...
	else
	{
		...
		printf("So luong doc gia dang ki: %d nguoi", Ns);
	}
	if (f)
	{
		fclose(f);
...
void numofsubs_gender()
{
	...
	else
	{
		...
		printf("So doc gia nam  : %d ", Male);
		printf("So doc gia nu   : %d ", FeMale);
		printf("So doc gia khac : %d ", Other);
	}
	if (f)
	{
		fclose(f);
